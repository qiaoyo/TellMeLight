...
const designBomRows = [
  ...
  ['D1-D6', '6', 'RGB_LED', 'S4-3528RGBTA-A', 'TellMeLight_Rev_A4:LED_RGB_TUOZHAN_S4-3528RGBTA-A_3.5x2.8mm', 'TUOZHAN S4-3528RGBTA-A', 'C2827321', 'Top', 'YELLOW', 'JLC color warning is expected for RGB; confirm color and orientation preview'],
  ...
  ['C18', '1', 'CAPACITOR', '1uF', 'Capacitor_SMD:C_0603_1608Metric', 'CL10A105KB8NNNC', 'C15849', 'Bottom', 'GREEN', 'RP2040 VREG_VOUT local capacitor'],
...
const jlcBomRows = [
  ...
  ['S4-3528RGBTA-A', 'D1,D2,D3,D4,D5,D6', 'TellMeLight_Rev_A4:LED_RGB_TUOZHAN_S4-3528RGBTA-A_3.5x2.8mm', 'C2827321'],
  ...
  ['1uF', 'C15,C16,C18', 'Capacitor_SMD:C_0603_1608Metric', 'C15849'],
...
function costRows() {
  return [
    ...
    ['Known Rev A4 JLC dry-run PCB quote', '5', '4-layer 76x56 target', 'estimate pending', 'Rev A2 96x74 quote was about RMB 60; Rev A4 should be lower after size reduction, but final quote must come from JLC'],
    ...
    ['U6 VLED TVS', '1', 'C436349', 'matched in dry run', 'Added to Rev A4 formal BOM/CPL'],
    ['R9 shell bleed', '1', 'C22935', 'matched in dry run', 'Added to Rev A4 formal BOM/CPL'],
    ['C17 shell shunt', '1', 'C57112', 'matched in dry run', 'Added to Rev A4 formal BOM/CPL'],
    ['R10 VLED source link', '1', 'C21189', 'matched in dry run', 'Added to Rev A4 formal BOM/CPL'],
  ];
}
...
await Promise.all([
  writeFile(join(bomDir, 'rev_a4_bom.csv'), csv(designBomRows), 'utf8'),
  writeFile(join(bomDir, 'rev_a4_jlc_bom.csv'), csv(jlcBomRows), 'utf8'),
  writeFile(join(bomDir, 'rev_a4_jlc_cpl.csv'), csv(cplRows), 'utf8'),
  writeFile(join(bomDir, 'rev_a4_cost_notes.csv'), csv(costRows()), 'utf8'),
  writeFile(join(notesDir, 'rev-a4-order-readiness.md'), readinessMarkdown(), 'utf8'),
]);

console.log('Generated Rev A4 order-review BOM, CPL, cost notes, and checklist.');
