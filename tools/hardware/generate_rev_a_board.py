...
def add_board_outline(board):
    # 96 mm x 74 mm near-square module, with chamfered visual corners for Rev A.
    outline = [
        (2.0, 0.0),
        (94.0, 0.0),
        (96.0, 2.0),
...
def add_diffuser_marks(board):
    ...
    add_text(board, "TellMeLight Rev A", 36, 5, pcbnew.F_SilkS, 1.1)
...
def place_components(board):
    ...
    for reference, x, y in led_positions:
        load_footprint(board, "LED_SMD", "LED_RGB_PLCC-6", reference, "RGB_LED", x, y, rotation=90)
...
def build_board(output_path):
    ...
    title.SetTitle("TellMeLight Rev A")
    title.SetDate("2026-05-30")
    title.SetRevision("A0")
