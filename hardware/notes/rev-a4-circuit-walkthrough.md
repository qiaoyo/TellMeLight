...
Rev A4 的目标是把 Rev A3 的 JLC 预览包推进到真实可制造 PCB。当前已经补上 RP2040 `VREG_VOUT` 的 1uF 本地电容 `C18`，并建立了 KiCad DSN -> Freerouting SES -> KiCad 导回的自动布线实验链路。A4 placement/fanout revision 之后，KiCad DRC 已达到 `0 error violations, 0 unconnected items`，还剩 9 个 `via_dangling` warning。现在它可以进入 JLC 预览检查，但还不应该直接付款；付款前必须人工确认 JLC DFM、0.45 mm outer / 0.30 mm drill 免费普通过孔、0.10 mm 线宽线距、SMT 方向和器件警告。
...
Rev A4 已经证明：BOM/CPL、JLC 器件选型、USB/VLED 保护、电源框架、RP2040/LP5024 fanout 和自动布线工具链方向成立。当前 PCB DRC 结果为 `0 error violations and 0 unconnected items`，并保留 9 个 `via_dangling` warning 供 JLC DFM 人工确认。

下一步不是继续改线，而是上传 JLC 做 preview / DFM / SMT orientation 检查。只有当 JLC 识别 4 层、76 mm x 56 mm、钻孔、Gerber、BOM/CPL 和方向都正确，并且没有把当前 0.45 mm outer / 0.30 mm drill 免费普通过孔或 0.10 mm 线宽线距报成不可接受工艺时，才可以考虑付款。
